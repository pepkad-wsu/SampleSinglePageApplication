...
namespace SampleSinglePageApplication.Web.Controllers
{
    ...
    public class DataController : ControllerBase
    {
		...
		public DataController(IDataAccess daInjection, IHttpContextAccessor httpContextAccessor, IEncryption enc, IHubContext<SignalRHub> hubContext)
		{
			da = daInjection;
        ...
        public async Task<ActionResult<DataObjects.User>> Authenticate(Guid id, DataObjects.Authenticate authenticate)
        {
            ...
            if(authenticate != null && !String.IsNullOrEmpty(authenticate.Username) && !String.IsNullOrEmpty(authenticate.Password)) {
                output = await da.Authenticate(id, authenticate.Username, authenticate.Password, true);
            }
        ...
        public ActionResult<DataObjects.BooleanResponse> Decrypt(DataObjects.SimplePost post)
        {
            ...
            if (CurrentUser.Admin) {
                string DecryptedText = encryption.Decrypt(post.SingleItem);
        ...
        public ActionResult<DataObjects.BooleanResponse> Encrypt(DataObjects.SimplePost post)
        {
            ...
            if (CurrentUser.Admin) {
                string Encrypted = encryption.Encrypt(post.SingleItem);
        ...
        public async Task<ActionResult<DataObjects.BooleanResponse>> SaveLanguage(Guid id, List<DataObjects.OptionPair> language)
        {
            if (CurrentUser.Admin) {
